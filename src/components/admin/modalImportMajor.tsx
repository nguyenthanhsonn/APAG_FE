...
const getImportRows = (result: ImportMajorsResult) =>
  result.previewMajors?.length
    ? result.previewMajors
    : result.createdMajors || [];
...
export default function ModalImportMajor({ isOpen, onClose, onSuccess }: ModalImportMajorProps) {
  const [result, setResult] = useState<ImportMajorsResult | null>(null);
  ...
  const handleDownloadTemplate = async () => {
    try {
      setDownloading(true);
      setErrorMsg('');
      const blob = await API_Admin.downloadMajorImportTemplate();
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', 'mau_nganh.xlsx');
      document.body.appendChild(link);
      link.click();
      link.remove();
      window.URL.revokeObjectURL(url);
    } catch (err: any) {
      setErrorMsg(getUserFriendlyError(err, 'Không thể tải file mẫu ngành. Vui lòng thử lại.'));
    } finally {
      setDownloading(false);
    }
  };
  ...
  const handleFileUpload = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await API_Admin.importMajors(file);
      setResult(res);
    } catch (err: any) {
      setSelectedFileName(file.name);
      setErrorMsg(getUserFriendlyError(err, 'Không thể đọc file ngành. Vui lòng kiểm tra lại file Excel.'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleConfirm = async () => {
    ...
    try {
      setConfirming(true);
      setErrorMsg('');
      const res = await API_Admin.confirmImportMajors({ importToken: result.importToken });
      setResult(res);
      setConfirmed(true);
      onSuccess?.();
    } catch (err: any) {
      setErrorMsg(getUserFriendlyError(err, 'Không thể xác nhận import ngành. Vui lòng tải lại file và thử lại.'));
    } finally {
      setConfirming(false);
    }
  };

  const rows = result ? getImportRows(result) : [];
  const errors = result?.errors || [];
  const canConfirm = !!result?.importToken && !confirmed && result.failedCount === 0;

  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/50 p-4 backdrop-blur-[2px]" onClick={handleClose}>
      <div className="relative w-full max-w-3xl rounded-2xl bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-gray-100 px-6 py-4">
          <div>
            <h2 className="text-base font-bold text-gray-900">Import ngành từ Excel</h2>
            <p className="mt-0.5 text-xs font-medium text-gray-500">
              File mẫu mới gồm Mã ngành, Tên ngành và Tên khoa.
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="max-h-[72vh] space-y-4 overflow-y-auto px-6 py-5">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-xs font-medium text-gray-500">
                  Dùng cột Tên khoa trong file, không cần chọn khoa ở giao diện.
                </p>
              </div>
              <button
                type="button"
          ...
          <div className="rounded-xl border border-gray-100 bg-gray-50/60 p-4">
            <h3 className="text-sm font-bold text-gray-900">2. Upload file để preview</h3>
              ...
              {loading ? (
                ...
                <>
                  ...
                  <p className="mb-3 text-xs font-medium text-gray-600">
                    {selectedFileName || 'Chọn file .xlsx hoặc .xls để hệ thống kiểm tra trước'}
                  </p>
                  <input
              ...
              {rows.length > 0 && (
                <div className="mt-4 rounded-xl border border-blue-100 bg-blue-50/40 p-4">
                  <div className="mb-3 flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                    <h4 className="text-sm font-bold text-blue-900">Danh sách ngành trong file Excel</h4>
                    ...
                    {rows.map((row, index) => (
                      <div key={`${row.code}-${index}`} className="rounded-xl border border-blue-100 bg-white p-3 text-xs">
                        ...
                        <div className="grid gap-2 sm:grid-cols-2">
                          ...
                          <div className="min-w-0">
                            <p className="text-[10px] font-bold uppercase text-gray-400">Khoa</p>
                            <p className="truncate text-gray-700">
                              {[row.facultyCode, row.facultyName || row.faculty?.name].filter(Boolean).join(' - ') || '—'}
                            </p>
                          </div>
        ...
        <div className="flex items-center justify-end gap-3 border-t border-gray-100 px-6 py-4">
          ...
          {result && !confirmed && (
            ...
            >
              ...
              {result.importToken ? `Xác nhận import ${result.successCount} ngành` : 'Thiếu importToken'}
            </button>
          )}
        </div>
      </div>
