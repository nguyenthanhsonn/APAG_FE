  ...
  return (
    <div className="mx-auto flex w-full max-w-7xl flex-1 flex-col gap-6 p-4 sm:p-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-xs flex items-center gap-4"><div className="p-3 bg-blue-50 text-blue-600 rounded-xl"><Users size={22} /></div><div><p className="text-xs text-gray-500 font-medium">Số phiếu</p><p className="text-xl font-bold text-gray-900">{totalCount} Phiếu</p></div></div>
        ...
        <div className="p-4 border-b border-gray-100 flex flex-col sm:flex-row gap-3 justify-between items-center bg-gray-50/50">
          <div className="flex flex-col sm:flex-row gap-3 items-center w-full sm:w-auto">
            <div className="relative w-full sm:w-72">
              ...
              <input type="text" placeholder="Tìm theo tên hoặc mã SV..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-9 pr-4 py-2 text-sm bg-white border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" />
            </div>
            <div className="flex items-center gap-2 w-full sm:w-auto">
              <Filter size={16} className="text-gray-400 shrink-0" />
              <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="px-3 py-2 text-sm bg-white border border-gray-200 rounded-xl focus:outline-none focus:border-blue-500">
          ...
          <table className="w-full text-left border-collapse text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? <tr><td colSpan={7} className="py-12 text-center text-sm text-gray-400">Đang tải dữ liệu...</td></tr> : !classId ? <tr><td colSpan={7} className="py-12 text-center text-sm text-gray-400">Tài khoản này chưa được gán lớp trưởng cho lớp nào.</td></tr> : filteredStudents.length === 0 ? <tr><td colSpan={7} className="py-12 text-center text-sm text-gray-400">Không có dữ liệu phiếu từ API.</td></tr> : filteredStudents.map((st) => (
                <tr key={st.evaluationId} className="hover:bg-blue-50/30 transition-colors">
                  <td className="py-3.5 px-4 font-mono font-semibold text-gray-700">{st.code}</td><td className="py-3.5 px-4 font-medium text-gray-900">{st.name}</td><td className="py-3.5 px-4 text-center font-bold text-blue-600">{st.score > 0 ? st.score : '-'}</td><td className="py-3.5 px-4 text-center">{st.rank !== '-' ? <span className="inline-block px-2.5 py-0.5 text-xs font-semibold bg-gray-100 text-gray-700 rounded-md">{st.rank}</span> : '-'}</td>
                  ...
                  <td className="py-3.5 px-4 text-gray-500 text-xs">{st.date}</td><td className="py-3.5 px-4 text-right"><div className="flex items-center justify-end gap-2">{st.rawStatus === 'submitted' && <button type="button" disabled={loading || st.score <= 0} onClick={() => handleApprove(st)} className="px-2.5 py-1 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-xs font-semibold transition cursor-pointer flex items-center gap-1 disabled:opacity-50"><FileCheck size={13} /> Duyệt</button>}</div></td>
                </tr>
              ))}
            </tbody>
          </table>
