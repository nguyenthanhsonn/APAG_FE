...
function buildFacultyStats(evaluations: AdminEvaluationItem[]): FacultyStat[] {
  ...
  evaluations.forEach((item) => {
    ...
    record.total += 1;
    if (String((item as any).status).toLowerCase() === 'finalized') record.completed += 1;
...
export function TrainingDeptDashboard() {
  ...
  const completedCount = evaluations.filter((item: any) => String(item.status).toLowerCase() === 'finalized').length;
  ...
  return (
    <div className="mx-auto flex w-full max-w-7xl flex-1 flex-col gap-6 p-4 sm:p-6">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between bg-white p-6 rounded-2xl border border-gray-100 shadow-xs">
        ...
        <button type="button" className="flex cursor-pointer items-center gap-2 self-start rounded-xl bg-brand-primary px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-red-700 sm:self-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-xs flex items-center gap-4"><div className="p-3 bg-brand-secondary/10 text-brand-secondary rounded-xl"><Users size={24} /></div><div><p className="text-xs text-gray-500 font-medium">Tổng số phiếu</p><p className="text-2xl font-bold text-gray-900">{totalStudents} Phiếu</p></div></div>
        ...
        <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-xs flex items-center gap-4"><div className="p-3 bg-brand-secondary/10 text-brand-secondary rounded-xl"><Building2 size={24} /></div><div><p className="text-xs text-gray-500 font-medium">Tổng số Khoa/Ban</p><p className="text-2xl font-bold text-brand-secondary">{facultyStats.length} Khoa</p></div></div>
      </div>

      <div className="bg-white rounded-2xl border border-gray-100 shadow-xs overflow-hidden">
        <div className="p-4 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
          <h2 className="font-bold text-gray-900 flex items-center gap-2"><FileSpreadsheet size={18} className="text-brand-secondary" /> Thống kê tiến độ đánh giá theo từng Khoa</h2>
          <span className="rounded-full border border-brand-secondary/15 bg-brand-secondary/10 px-3 py-1 text-xs font-semibold text-brand-secondary">Dữ liệu API</span>
        </div>
